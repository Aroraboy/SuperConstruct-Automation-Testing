/**
 * Owner Accepts Invitation — onboards with company creation + accepts project
 *
 * Flow (from codegen):
 ...
 *
 * Key differences from member accept:
 *   - Has a COMPANY CREATION step (step 6)
 *   - After onboarding, must click "Accept Invitation" on the project page
 *
 * Prerequisites: Run gc-invite-owner first
 * Reads owner data from .auth/gc-invited-owner.json
 * Reads invite link + OTP from Gmail via IMAP
 *
 * Usage: npm run test:gc-owner-accept
 */
...
const ACCEPTED_OWNER_PATH = path.join(__dirname, '..', '..', '.auth', 'gc-accepted-owner.json');
...
function loadOwnerData() {
  if (!fs.existsSync(OWNER_PATH)) {
    throw new Error('Invited owner not found. Run "npm run test:gc-invite-owner" first.');
  }
  return JSON.parse(fs.readFileSync(OWNER_PATH, 'utf-8'));
}

test.describe.serial('Owner Accepts Invitation', () => {
  let owner;

  test.beforeAll(async () => {
    owner = loadOwnerData();
    console.log(`\n[SETUP] Owner: ${owner.email}`);
    console.log(`[SETUP] Name: ${owner.firstName} ${owner.lastName}`);
  });

  test('Owner accepts invitation and onboards', async ({ browser }) => {
    ...
    const inviteData = await getInviteDataFromGmail(owner.email, 90000);
    ...
    try {
      ...
      await page.getByRole('textbox', { name: 'Phone Number' }).fill('4567891230');
      console.log('   [DONE] Phone: 4567891230');
      ...
      // ---- STEP 7: Company page (Owner-specific) ----
      console.log('\n[STEP 7] Filling company details (Owner step)...');

      // Company name
      const runNumber = getRunNumber();
      const companyName = `Sparks Construction ${runNumber}`;
      ...
      await page.screenshot({ path: `test-results/owner-accepted-${Date.now()}.png` }).catch(() => {});
      ...
      const authDir = path.dirname(ACCEPTED_OWNER_PATH);
      if (!fs.existsSync(authDir)) fs.mkdirSync(authDir, { recursive: true });
      fs.writeFileSync(ACCEPTED_OWNER_PATH, JSON.stringify(acceptedOwner, null, 2));
      console.log(`\n[SAVED] Accepted owner -> .auth/gc-accepted-owner.json`);
      console.log('\n[COMPLETE] Owner onboarded + accepted project invitation!\n');
